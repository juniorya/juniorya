...
typedef struct
{
    ...
    vision_config vision_cfg;          /**< Vision system configuration. */
} main_launch_config;
...
static void control_process_entry(void *arg)
{
    ...
    const char *xml_path = getenv("KEBA_ECAT_XML");
    if ((xml_path != NULL) && ethcat_load_drives_from_xml(xml_path, &master_cfg))
    {
        log_printf(LOG_INFO, "Loaded EtherCAT XML from %s", xml_path);
    }
...
static void auxiliary_process_entry(void *arg)
{
    ...
    if (!vision_init(&vision, &cfg.vision_cfg))
    {
        log_printf(LOG_ERROR, "Vision initialisation failed");
    }
    else
    {
        for (int i = 0; i < 50; ++i)
        {
            if (vision_process_frame(&vision))
            {
                q16_16 error = 0;
                if (vision_get_alignment_error(&vision, &error))
                {
                    opcua_server_publish_alignment(&server, error);
                }
            }
            opcua_server_step(&server);
            osal_sleep_ms(100U);
        }
        vision_shutdown(&vision);
    }
...
int main(int argc, char *argv[])
{
    ...
    vision_default_config(&launch_cfg.vision_cfg);
