...
#if ENABLE_BRACHISTO
static void test_brachistochrone(void)
{
    vec3_q16 start = vec3_from_float(0.0f, 0.0f, 100.0f);
    vec3_q16 end = vec3_from_float(150.0f, 50.0f, 20.0f);
    spl_plan_t plan;
    assert(spl_make_brachistochrone(&start, &end, &plan));
    vec3_q16 pos;
    vec3_q16 deriv;
    assert(spl_sample_arc(&plan, q16_from_float(1.0f), &pos, &deriv));
    assert(q16_abs(q16_sub(pos.x, end.x)) < q16_from_float(0.1f));
    assert(q16_abs(q16_sub(pos.y, end.y)) < q16_from_float(0.1f));
    assert(q16_abs(q16_sub(pos.z, end.z)) < q16_from_float(0.1f));
    assert(q16_to_float(plan.length) > 0.0f);
}
#endif
