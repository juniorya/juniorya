                 ...
                 ethcat_master *master)
{
    ...
    for (size_t axis = 0; axis < motion->master->config.axis_count && axis < ETHCAT_MAX_AXES; ++axis)
    {
        ethcat_master_set_operation_mode(motion->master, axis, CIA402_MODE_CSP);
    }
    return true;
}
...
void motion_step(motion_controller *motion)
{
    ...
    if (motion->sample_index < motion->trajectory.count)
    {
        ...
        ethcat_master_set_position(motion->master, 0, pos.x);
        ethcat_master_set_position(motion->master, 1, pos.y);
        ethcat_master_set_position(motion->master, 2, pos.z);
        motion->sample_index++;
    }
}
