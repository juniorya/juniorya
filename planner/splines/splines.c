...
#if ENABLE_BRACHISTO
static bool brachisto_solve(float horizontal, float drop, float *theta_out, float *radius_out)
{
    if (horizontal <= 0.0f || drop <= 0.0f)
    {
        return false;
    }
    const float ratio = horizontal / drop;
    const float pi = 3.1415926535f;
    float guesses[3];
    guesses[0] = fmaxf(0.5f, ratio * 1.5f);
    guesses[1] = fmaxf(1.0f, pi);
    guesses[2] = fmaxf(1.0f, ratio * 3.0f);
    for (size_t g = 0U; g < (sizeof(guesses) / sizeof(guesses[0])); ++g)
    {
        float theta = guesses[g];
        for (int i = 0; i < 48; ++i)
        {
            float sin_t = sinf(theta);
            float cos_t = cosf(theta);
            float f = (theta - sin_t) - ratio * (1.0f - cos_t);
            if (fabsf(f) < 1e-6f)
            {
                break;
            }
            float df = (1.0f - cos_t) - ratio * sin_t;
            if (fabsf(df) < 1e-6f)
            {
                theta += 0.5f;
                continue;
        ...
        float sin_t = sinf(theta);
        float cos_t = cosf(theta);
        float residual = (theta - sin_t) - ratio * (1.0f - cos_t);
        if (fabsf(residual) > 1e-4f)
        {
            continue;
        }
        float denom = 1.0f - cos_t;
        if (denom <= 1e-6f)
        {
            continue;
        }
        float radius = drop / denom;
...
static vec3_q16 eval_brachisto(const spl_plan_t *s, float t, vec3_q16 *dxyz)
{
    if (t < 0.0f)
    {
        t = 0.0f;
    ...
    float theta_max = to_float(s->brachisto_theta);
    float radius = to_float(s->brachisto_radius);
    float theta = theta_max * t;
    float sin_t = sinf(theta);
    float cos_t = cosf(theta);
    float x_local = radius * (theta - sin_t);
    float y_local = radius * (1.0f - cos_t);
    float dx_dtau = radius * (1.0f - cos_t) * theta_max;
    float dy_dtau = radius * sin_t * theta_max;

    vec3_q16 origin = s->control[0];
    vec3_q16 horiz = s->brachisto_dir_h;
    vec3_q16 vert = s->brachisto_dir_v;

    float ox = to_float(origin.x);
    float oy = to_float(origin.y);
    float oz = to_float(origin.z);
    float hx = to_float(horiz.x);
    float hy = to_float(horiz.y);
    float hz = to_float(horiz.z);
    float vx = to_float(vert.x);
    float vy = to_float(vert.y);
    float vz = to_float(vert.z);

    float px = ox + x_local * hx + y_local * vx;
    float py = oy + x_local * hy + y_local * vy;
    float pz = oz + x_local * hz + y_local * vz;

    if (dxyz != NULL)
    {
        float dx = dx_dtau * hx + dy_dtau * vx;
        float dy = dx_dtau * hy + dy_dtau * vy;
        float dz = dx_dtau * hz + dy_dtau * vz;
        *dxyz = vec_from_float(dx, dy, dz);
    }

    return vec_from_float(px, py, pz);
}
#endif
...
static float derivative_magnitude(const spl_plan_t *s, float t)
{
    ...
    switch (s->type)
    {
        ...
        case SPL_NURBS3:
#if ENABLE_BRACHISTO
        case SPL_BRACHISTOCHRONE:
#endif
        default:
#if ENABLE_BRACHISTO
            if (s->type == SPL_BRACHISTOCHRONE)
            {
                (void)eval_brachisto(s, t, &deriv);
            }
            else
            {
...
bool spl_make_brachistochrone(const vec3_q16 *start, const vec3_q16 *end, spl_plan_t *out)
{
    ...
    memset(out, 0, sizeof(*out));
    float sx = to_float(start->x);
    float sy = to_float(start->y);
    float sz = to_float(start->z);
    float ex = to_float(end->x);
    float ey = to_float(end->y);
    float ez = to_float(end->z);
    float dx = ex - sx;
    float dy = ey - sy;
    ...
    float theta;
    float radius;
    if (!brachisto_solve(horizontal, drop, &theta, &radius))
    {
        return false;
    }
    ...
    out->control[0] = *start;
    out->control[1] = *end;
    out->brachisto_theta = from_float(theta);
    out->brachisto_radius = from_float(radius);
    if (horizontal > 1e-6f)
    {
        out->brachisto_dir_h = vec_from_float(dx / horizontal, dy / horizontal, 0.0f);
    }
    else
    {
        out->brachisto_dir_h = vec_from_float(1.0f, 0.0f, 0.0f);
    }
    out->brachisto_dir_v = vec_from_float(0.0f, 0.0f, -1.0f);
    out->length = from_float(4.0f * radius * (1.0f - cosf(theta * 0.5f)));
    return true;
}
#endif
