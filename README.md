...
To enable the full graphical workflow install Qt 6 with Widgets and OpenGL
support, then configure a CMake build:

```bash
cmake -S . -B build-qt -DBUILD_GUI=ON
cmake --build build-qt
```

The resulting `baget_ide` executable launches the 3D viewport, spline editor and
project manager. Use `-DBUILD_GUI=OFF` when Qt is unavailable; console tools and
the legacy simulator remain functional.
...
- `baget_ide` — the Qt 6 application with interactive 3D viewport, spline
  editor, planner inspector and project manager (`*.bagetproj`). Use the toolbar
  to rebuild previews, open/save projects and inspect camera presets.
- `cnc_ide` — the original console workflow that converts ``.cncp`` projects
  into sampled trajectories and produces OBJ/PPM/JSON/CSV artefacts for headless
  regression.
